...
// sushi router addr: 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506
...
describe("main", () => {
    ...
    context("withdraw", () => {
        ...
        it("should not allow withdrawal before adminswap", async () => {

            let balanceOfLp = (await lp.balanceOf(main.address)).toString();
            expect(balanceOfLp).to.be.eq('0')

            await expect(main.connect(USER1).withdraw('0')).to.be.revertedWith("withdraw locked");
        });
    });

    context("setAdminSwap", () => {
        it("should expect admin address to be a contract address", async () => {
            await expect(main.setAdminSwap(USER1.address)).to.be.revertedWith("Not a contract address");

            await main.setAdminSwap(admin.address)
            expect(await main.adminSwapContract()).to.be.eq(admin.address)
        });
    });

    context("adminSwap", () => {
        it("should expect main contract to have a lptoken balance", async () => {
